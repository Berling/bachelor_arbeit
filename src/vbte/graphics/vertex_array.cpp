...
namespace vbte {
	namespace graphics {
		...
		void vertex_array::bind_vertex_attribute(const vertex_buffer& buffer, uint32_t index, int32_t size, GLenum type, bool normalized, size_t stride, intptr_t offset) const noexcept {
			buffer.bind();
			glEnableVertexAttribArray(index);
			glVertexAttribPointer(index, size, type, normalized, stride, reinterpret_cast<GLvoid*>(offset));
		}

		void vertex_array::bind_vertex_attributei(const vertex_buffer& buffer, uint32_t index, int32_t size, GLenum type, size_t stride, intptr_t offset) const noexcept {
			buffer.bind();
			glEnableVertexAttribArray(index);
