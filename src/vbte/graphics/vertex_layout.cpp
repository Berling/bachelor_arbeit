...
namespace vbte {
	namespace graphics {
		void vertex_layout::emplace_back(const std::string& name, uint32_t size, GLenum type, bool normalized, size_t stride, intptr_t offset, int index, int divisor) {
		...
		void vertex_layout::emplace_back(const std::string& name, uint32_t size, GLenum type, bool normalized, size_t stride, intptr_t offset, intptr_t attribute_size, int factor, int index, int divisor) {
			...
			for (auto& attribute : vertex_attributes_) {
				for (auto j = 0; j < attribute.factor; ++j) {
					...
					if (attribute.type == GL_INT) {
						vertex_array.bind_vertex_attributei(*(buffers[attribute.index]), i, attribute.size, attribute.type, attribute.stride, attribute.offset + j * attribute.component_offset);
					} else {
						vertex_array.bind_vertex_attribute(*(buffers[attribute.index]), i, attribute.size, attribute.type, attribute.normalized, attribute.stride, attribute.offset + j * attribute.component_offset);
					}
