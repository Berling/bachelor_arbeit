...
public:
  buffer(GLenum usage) : usage_(usage) {
    glGenBuffers(1, &id_);
    if (id_ == 0) {
  ...
  buffer(size_t size, const void *data, GLenum usage) noexcept : usage_{usage} {
    glGenBuffers(1, &id_);
    bind();
  ...
  buffer(buffer &&rhs) noexcept : id_(rhs.id_), usage_(rhs.usage_) {
    rhs.id_ = 0;
  }
  ...
  void bind() const noexcept { bind(target); }
